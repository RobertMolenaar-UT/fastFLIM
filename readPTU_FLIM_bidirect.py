...
@njit
def get_flim_data_stack_static(sync, tcspc, channel, special, header_variables, bi_shift=0):
    ...
    ImgHdr_bidirect           = header_variables[8]
        ...
        for event in range(read_data_range+1):
            ...
            if (isPhoton and insideLine and insideFrame):
                ...
                if (currentLine % 2) == 1 and ImgHdr_bidirect == 1:
                    ...
                    #even lines biderect - all lines mono directional
...
#                 print(tagName, tagValue)
    ...
    def get_flim_data_stack(self): 
        ...
        # Check
            ...
            header_variables  = np.array([self.head["ImgHdr_Ident"], self.head["MeasDesc_Resolution"],self.head["MeasDesc_GlobalResolution"],self.head["ImgHdr_PixX"], self.head["ImgHdr_PixY"], self.head["ImgHdr_LineStart"],self.head["ImgHdr_LineStop"], self.head["ImgHdr_Frame"], self.head["ImgHdr_BiDirect"]],dtype = np.uint64)
        ...
        flim_data_stack = get_flim_data_stack_static(sync, tcspc, channel, special, header_variables, shift)
...
@njit
def get_lifetime_image(flim_data_stack,channel_number,timegating_start1,timegating_stop1,meas_resolution,estimated_irf=0):
    """
    Flime data sstack in 2D lifetime data out for the selected channel,     
    
    Parameters
    ----------
    flim_data_stack : TYPE
        FLIM data, extracted from  flim_data_stack, intensity_image = ptu_file.get_flim_data_stack().
    channel_number : TYPE
        DESCRIPTION. channel 1 =0 
    timegating_start1 : TYPE
        DESCRIPTION. timegate start  in integer
    timegating_stop1 : TYPE
        DESCRIPTION. timegate stop integer
    meas_resolution : TYPE
    ...
    """
    #Work data is extracted TimeGates from the Flim_data_stack and is the the real used data 
    
    work_data = flim_data_stack[:,:,channel_number,timegating_start1:timegating_stop1]
        
    #Make a weigthing scaling array prefereble as the same length from 0 to length of work data startinf from zero to length of TAC 
